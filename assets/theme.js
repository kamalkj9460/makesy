           ...
           /* harmony import */var __WEBPACK_IMPORTED_MODULE_1__helper_Responsive__ = __webpack_require__(1);
            ...
            var Popover = function () {
                function Popover(element, options) {
                    ...
                    this.PageOverlay2Element = document.querySelector('.PageOverlay2');
                ...
                _createClass(Popover, [{
                    ...
                    value: function open() {
                        ...
                        if (__WEBPACK_IMPORTED_MODULE_1__helper_Responsive__["default"].matchesBreakpoint('lap-and-up')) {
                            ...
                            if (this.showOverlay) {
                                this.PageOverlay2Element.classList.add('is-visible');
                                this.PageOverlay2Element.addEventListener('click', this._closeListener);
                            }
                        }
                    ...
                    value: function close() {
                        ...
                        if (__WEBPACK_IMPORTED_MODULE_1__helper_Responsive__["default"].matchesBreakpoint('lap-and-up')) {
                            ...
                            this.PageOverlay2Element.classList.remove('is-visible');
                            this.PageOverlay2Element.removeEventListener('click', this._closeListener);
                        }
            ...
            var ProductVariants = function () {
                ...
                }, 
                {
                    ...
                    value: function _attachListeners() {
                        ...
                        this.delegateElement.on('click', '[data-action="decrease-quantity"]', function () { return false });
                        this.delegateElement.on('click', '[data-action="increase-quantity"]', function () { return false });
                        this.delegateElement.on('change', '[name="quantity"]', this._validateQuantity.bind(this));

                        // Hook when a radio button change
                        this.delegateElement.on('change', '.SizeSwatch__Radio', this._onOptionChanged.bind(this));
                        this.delegateElement.on('click', '.ColorSwatch__Radio', this._onOptionChanged.bind(this));
                    }
                ...
                }, 
                {
                    ...
                    value: function _updateProductPrices(newVariant, previousVariant) {
                        var variant_title = $('option[selected="selected"]') ? $('option[selected="selected"]').attr("data_title") : "";
                        if (variant_title.toLowerCase() == "single unit" && $('.QuantitySelector__CurrentQuantity').val() > 11)
                            $('.QuantitySelector__CurrentQuantity').val('11');
                        ...
                        if (!newVariant) {
                            ...
                            // This may need some editing if we ever need to start showing comparison pricing
                ...
                {
                    key: '_getClipPrice',
                    value: function _getClipPrice() {
                        var ClipValue = ""; var PackSize = "";
                        PackSize = $('#pack-size').html();
                        ClipValue = $('[data-clip-variant]').attr('data-clip-variant') ? $('[data-clip-variant]').attr('data-clip-variant') : "";
                        ClipValue = ClipValue != "" ? ClipValue.toLowerCase() : "";
                        var clipPriceVal = 0.00;
                        if (ClipValue == 'pre-inserted') {
                            clipPriceVal = window.flatClipPreinsertedPrice;
                        }
                        if (ClipValue == 'separate') {
                            clipPriceVal = window.flatClipSeparatePrice;
                        }

                        if (PackSize == "1000") {
                            if (ClipValue == 'pre-inserted') {
                                clipPriceVal = window.flatClipPreinsertedPrice_1000;
                            } else if (ClipValue == 'separate') {
                                clipPriceVal = window.flatClipSeparatePrice_1000;
                            }
                        }
                        ...
                        return Number(clipPriceVal * 100);
                    }
                },
                {
                        ...
                        if (!newVariant) {
                            ...
                            if (newVariant['available']) {
                                ...
                                if (undefined === this.options['showPriceInButton'] || this.options['showPriceInButton']) {
                                    var tags = $('.ProductMeta').attr("data-product-tags");
                                    var prc = newVariant['price'];
                                    var discountSelector = get_selector_from_metafield();
                                    if (document.querySelector('div.QuantitySelector.QuantitySelector--large input')) {
                                        qty_selector = document.querySelector('div.QuantitySelector.QuantitySelector--large input');
                                    } else if (document.querySelector('#Quantity')) {
                                        qty_selector = document.querySelector('#Quantity');
                                    }
                                    qty = qty_selector.value;
                                    var discount = getDiscountAmount(discountSelector, qty, $('.product_tier_data').val());
                                    window.discount = discount;
                                    prc = Number(prc - prc * discount);
                                    prc = Number(prc + this._getClipPrice())
                                    if (tags.includes("pre-order")) {
                                        newButton.innerHTML = '<span>PRE-ORDER</span><span class="Button__SeparatorDot"></span><span data-money-convertible>' + __WEBPACK_IMPORTED_MODULE_4__helper_Currency__["default"].formatMoney(prc, window.theme.moneyFormat) + '</span>';
                                    }
                                    else {
                                        newButton.innerHTML = '<span>' + window.languages.productFormAddToCart + '</span><span class="Button__SeparatorDot"></span><span data-money-convertible>' + __WEBPACK_IMPORTED_MODULE_4__helper_Currency__["default"].formatMoney(prc, window.theme.moneyFormat) + '</span>';
                                    }
                                } else {
                ...
                // {
                //     key: '_saleItem',
                //     value: function (dataArr, target, id, minPrice, saleQuantity) {
                //         console.log('dataArr:', dataArr)
                        
                //         /* Added 
                //              * comment out below on earth day
                //         */
                //         // if(!/wick/.test(window.location.pathname)) return;

                //         // Test if Sale item is already in cart
                //         const isAlreadyAdded = Array.from(document.querySelectorAll('.Cart__ItemList [data-variant-id]')).some(el => {
                //             const variantId = el.dataset.variantId
                //             const regex = new RegExp(id, 'g')
                //             return regex.test(variantId)
                //         })

                //         if (isAlreadyAdded) return
                        
                //         /* Get total cart price if title has word wick in it 
                //         *  Added 
                //         * comment out currentCartPrice & remove comments on cartBtn and currentCartPrice on earth day
                //         */
                        
    
                //         const totalPrice = Number(target.querySelector('[data-money-convertible]').textContent.replace(/\$/, ''))
                //         console.log('totalPrice:', totalPrice)
                //         // const itemQuantity = parseInt(target.closest('[action="/cart/add"]').querySelector('[name="quantity"]').value);
                ...
                // },
                {
                    ...
                    value: function (dataArr, target, id, minPrice, saleQuantity) {
                        ...
                        */
                        ...
                        let q = 0 
                        switch(true) {
                            case totalPrice >= 100 && totalPrice < 200: q = 1
                                break;
                            case totalPrice >= 200 && totalPrice < 300: q = 2
                                break;
                            case totalPrice >= 300 && totalPrice < 400: q = 3
                                break;
                            case totalPrice >= 400 && totalPrice < 500: q = 4
                                break;
                            case totalPrice >= 500 && totalPrice < 600: q = 5
                                break;
                            case totalPrice >= 600 && totalPrice < 700: q = 6
                                break;
                            case totalPrice >= 700 && totalPrice < 800: q = 7
                                break;
                            case totalPrice >= 800 && totalPrice < 900: q = 8
                                break;
                            case totalPrice >= 900 && totalPrice < 1000: q = 9
                                break;
                            case  totalPrice > 1000: q += 10
                                break;
                        }
                ...
                },
                {
                    ...
                    value: function _addToCart(event) {
                        ...
                        /* ------------------------------------------------ 
                        @DO - Custom and Ready wicks "bundle" logic on ATC
                        Adds corresponding clip variant, if selected
                        Matches clips quantity to wick quantity and adds to cart
                        Adds to cart a fee if wick is custom
                        All line items coming from a PDP with #clip-selector will have property '_custom-wick-bundle' set to the bundleId
                        ------------------------------------------------ */
                        var clipSelector = this.element.querySelector('#clip-selector');
                        if (clipSelector) {
                            var collection = clipSelector.getAttribute('data-collection');
                            var formElement = this.element.querySelector('form[action*="/cart/add"]');
                            var formQuantity = parseInt(formElement.querySelector('[name="quantity"]').value);
                            if (window.currentVariant) {
                                var wickVariant = window.currentVariant;
                            } else {
                                var wickVariant = _this7.currentVariant;
                            }

                            var clipVariant = null;
                            var clipSelection = clipSelector.querySelector('.ProductForm__SelectedValue') ? clipSelector.querySelector('.ProductForm__SelectedValue').getAttribute('data-clip-variant') : "None";
                            var productType = clipSelector.querySelector('.ProductForm__SelectedValue').getAttribute('data-product-type');
                            var packSize = $('#pack-size').length ? $('#pack-size').html() : "";
                            clipVariant = getVariantIdWicks(clipSelection, productType, packSize, wickVariant);
                            // BundleId is used to identify and manage multiple bundles on the cart
                            var bundleId = wickVariant.id + '-' + clipVariant;
                            var bundleItems = [];
                            //Adds wicks
                            ...
                            // Adds setup fee only for custom wicks and quantities < 1000

                            if (collection == "virtual-custom-wicks" || collection == "X Wicks" || productType == "custom-wicks") {
                                if (formQuantity < 1000 && packSize != "1000") {
                                    bundleItems.push({
                                        quantity: 1,
                                        id: 35935883853978,
                                        properties: {
                            ...
                            /* 
                            * Params:
                            *    current array, target element, sale variant id, sale threshhold, quantity of sale item
                            */
                            // this._saleItem(bundleItems, event.target, 41866862362778, 0, 10)
                            this._saleItem(bundleItems, event.target, 41866862362778, 100, 1)
                            console.log('bundleItems:', bundleItems)

                            fetch(window.routes.cartAddUrl + '.js', {
                                body: JSON.stringify({ items: bundleItems }),
                                // body: JSON.stringify({ items: [{quantity: 1, id: 41866862362778}] }),
                                    ...
                                    response.json().then(function (content) {
                                        ...
                                        if (v == 422) {
                                            ...
                                            jQuery.getJSON('/cart.js', {
                                                ...
                                                errorMessageElement.innerHTML = "Only " + document.getElementById('inv_qnt01_' + _this7.currentVariant.id + '').innerHTML + " left on stock";
                                                let items = cart.items;
                                                for (var i = 0; i < items.length; i++) {
                                                    if (items[i].id == _this7.currentVariant.id) {
                                                        var g = Number(document.getElementById('inv_qnt01_' + _this7.currentVariant.id + '').innerHTML) - Number(items[i].quantity);
                                                        errorMessageElement.innerHTML = "Only " + g + " left on stock";
                                                    }
                            ...
                            let formData = [__WEBPACK_IMPORTED_MODULE_2__helper_Form__["default"].serialize(formElement)]

                            /* Sales function for free item */

                            /* 
                            * Params:
                            *    current array, target element, sale variant id, sale threshhold
                            */
 
                            // this._saleItem(formData, event.target, 41866862362778, 0, 10)
                            // this._saleItem(formData, event.target, 41866862362778, 100, 1)
                            ...
                            fetch(window.routes.cartAddUrl + '.js', {
                                ...
                                if (response.ok) {
                                    ...
                                    /* 
                                        CART ICON UPDATE QUANTITY FROM CART ITEMS 
                                        Updates cart icon when items are added from product page. Brad's addtion 3-15-2022  
                                    */
                                    const cartIcon = document.querySelector('[data-action="open-drawer"] .small')
                                    response.json().then(data => {

                                        const newQuantity = data.items[0].quantity
                                        const newVariantId = data.items[0].variant_id
                                        const cartQuantityData = document.querySelectorAll('[data-line-item-quantity]')

                                        /* Gets all line items from cart and adds the total together. */
                                        const cartTotal = Array.from(cartQuantityData).reduce((prev, data) => {
                                            const quantity = data.dataset.variantId != newVariantId ? Number(data.dataset.lineItemQuantity) : 0
                                        ...
                                        setTimeout(() => {
                                            Array.from(document.querySelectorAll('.CartItem__Title.Heading a')).forEach(link => {
                                                    if (/variant=41866862362778/.test(link.href)) {
                    ...
                    value: function _decreaseQuantity(event, target) {
                        ...
                        updatePriceATC(discount = window.discount, target.nextElementSibling.value);
                        const e = new Event("change");
                        target.nextElementSibling.dispatchEvent(e);
                    }
                    ...
                    value: function _increaseQuantity(event, target) {
                        ...
                        updatePriceATC(target.previousElementSibling.value);
                        const e = new Event("change");
                        target.previousElementSibling.dispatchEvent(e);
                    }
            ...
            var Collapsible = function () {
                ...
                _createClass(Collapsible, [{
                    ...
                    value: function _attachListeners() {
                        ...
                        this.domDelegate.on('click', '[data-action="toggle-collapsible"]', function () { return false });
                    }
                    ...
                    value: function _openModal(modal) {
                        ...
                        fastdom.mutate(function () {
                            ...
                            if (!_this17.activeModal.classList.contains('Modal--fullScreen')) {
                                _this17.PageOverlay2Element.classList.add('is-visible');
                                _this17.PageOverlay2Element.addEventListener('click', _this17._closeListener);
                            }
                        });
                    }
                }, {
                    ...
                    value: function _closeModal() {
                        ...
                        fastdom.mutate(function () {
                            ...
                            if (!_this18.activeModal.classList.contains('Modal--fullScreen')) {
                                _this18.PageOverlay2Element.classList.remove('is-visible');
                                _this18.PageOverlay2Element.removeEventListener('click', _this18._closeListener);
                            }
                    ...
                    value: function _closeSearch() {
                        ...
                        var onTransitionEnd = function onTransitionEnd(event) {
                            if (event.propertyName === 'visibility') {
                                ...
                                _this21.PageOverlay2Element.removeEventListener('transitionend', onTransitionEnd);
                            }
                        };

                        this.PageOverlay2Element.addEventListener('transitionend', onTransitionEnd);
                        this.PageOverlay2Element.classList.remove('is-visible');
                    }
         ...
         /***/function (module, __webpack_exports__, __webpack_require__) {
           ...
           /* harmony import */var __WEBPACK_IMPORTED_MODULE_2__components_ShippingEstimator__ = __webpack_require__(20);
            ...
            var CartSection = function () {
                ...
                {
                    key: '_updateSaleItem',
                    value: function(formData, target, saleID, saleAmount, quantity, saleItemQuantity) {

                        const currentId = target.getAttribute('data-line-id')
                        const currentTarget = target.parentNode.querySelector('input[data-variant-id]')
                        
                        /* Added */
                        let currentSaleQuantity = document.querySelector('input[data-variant-id=" '+ saleID +'"]') 
                        currentSaleQuantity = currentSaleQuantity ? currentSaleQuantity.value : 1
    
                        const totalPrice = Array.from(document.querySelectorAll('[data-line-item-quantity]')).reduce((obj, el) => {
                            const title = el.closest('.CartItem__Info').querySelector('a').textContent
                            const isWick = /wick/i.test(title)

                            /* Added 
                             * comment out itemPrice below & remove comments on itemPrice below on earth day
                             *  After sale is done remove itemPrice below and title and isWick above
                            */
                            const itemPrice = isWick ? Number(el.closest('.CartItem__Info')
                                .querySelector('.CartItem__Price.Price').textContent.replace(/\$/, '')) : 0

                            // const itemPrice = Number(el.closest('.CartItem__Info')
                            //     .querySelector('.CartItem__Price.Price').textContent.replace(/\$/, ''))

                            /*
                            - Current target == item, then quantity is changed from number between minus and plus buttons
                            - other wise minus or plus was clicked
                            */
                            const newQuantity = currentTarget === el ? quantity : Number(el.value)
                            obj += (newQuantity * itemPrice)
                            return obj
                        }, 0)
                        ...
                        /* quantity clicked from sale item */
                        if (new RegExp(saleID, 'g').test(currentId)) {
                            // return formData['updates'] = {[saleID]:  saleItemQuantity}
                            return formData['updates'] = {[saleID]:  q}
                        }
                        if (totalPrice < saleAmount) {
                            return formData['updates'][saleID] = 0
                ...
                // },
                {
                    ...
                    value: function _updateItemQuantity(event, target) {
                        ...
                        /* 
                            UPDATE CART ICON NAV WITH CART QUANTITY
                            dirty hack due to not finding where the return data for cart update is Brad additon 3-14-2022 
                        */
                        const cartIcon = document.querySelector('[data-action="open-drawer"] .small')
                        setTimeout(() => {
                            const cartQuantityData = document.querySelectorAll('[data-line-item-quantity]')
                            ...
                            cartTotal > 99 ? cartIcon.classList.add('small-many') : cartIcon.classList.remove('small-many')
                            cartIcon.textContent = cartTotal
                            document.querySelector('[data-action="open-drawer"] .small').textContent = cartTotal
                        ...
                        /* ------------------------------------------------ 
                        @DO - Custom and Ready wicks "bundle" logic on updateItemQuantity.
                        Bundles are sets of wicks + clips (optional) + setup fee (custom wick only)
                        Logic below for updating quantities as a group.
                        ------------------------------------------------ */

                        // Checks if the cart (cart-items.liquid) includes a bundle. There are different elements that can include the bundleId at the data-property field
                        var bundleId = target.getAttribute('data-property');
                        if (bundleId) {
                            var feefound = false;
                            var clipTrigger = false;
                            var triggerVariantId = target.getAttribute('data-variant-id').trim();
                            if (triggerVariantId == "36970618192026" || triggerVariantId == "35920743956634" || triggerVariantId == "36970618192026") {
                                clipTrigger = true;
                            }

                            var bundleLineItems = this.element.querySelectorAll('[data-property]');
                            var nonBundleLineItems = this.element.querySelectorAll('.QuantitySelector__CurrentQuantity.item');

                            /* Submitting an array of numbers to /cart/update.js provided the size of the array matches the number of line items in the cart. 
                            Each number in the array sets the quantity for the corresponding line item in the cart 
                            https://shopify.dev/docs/themes/ajax-api/reference/cart#post-cart-update-js */
                            var updateArray = [];

                            /* Adding non-bundle line-items to updateArray */
                            Array.prototype.forEach.call(nonBundleLineItems, function (el, i) {
                                var lineItemIndex = parseInt(el.getAttribute(['data-line-item-index']));
                                var lineItemQuantity = parseInt(el.getAttribute(['value']));
                                updateArray[lineItemIndex] = lineItemQuantity;
                            });

                            // Only removing clips - Quantity selectors are disabled so logic below only considers removals
                            if (clipTrigger) {
                                Array.prototype.forEach.call(bundleLineItems, function (el, i) {
                                    var lineItemIndex = parseInt(el.getAttribute(['data-line-item-index']));
                                    var itemVariantId = parseInt(el.getAttribute(['data-variant-id']));
                                    var itemBundleId = el.getAttribute(['data-property']);
                                    var itemCurrentQty = parseInt(el.getAttribute(['data-line-item-qty']));
                                    if (itemBundleId == bundleId) {
                                        if (itemVariantId == triggerVariantId) {
                                            // At the moment, _updateItemQuantity for bundle items only gets called on Remove
                                            updateArray[lineItemIndex] = 0
                                        } else {
                                            updateArray[lineItemIndex] = itemCurrentQty;
                                        }
                                    } else {
                                        updateArray[lineItemIndex] = itemCurrentQty;
                                    }
                                ...
                                Array.prototype.forEach.call(bundleLineItems, function (el, i) {
                                    ...
                                    var itemCurrentQty = parseInt(el.getAttribute(['data-line-item-qty']));
                                    if (itemBundleId == bundleId) {
                                        // @DO - TODO: Fix adding another fee when adding new line items with same variants already existing on the cart

                                        // Checks quantity to decide on setup fee 
                                        if (itemVariantId == 35935883853978) {
                                            feefound = true;
                                            if (quantity >= 1000 || quantity < 1) {
                                                updateArray[lineItemIndex] = 0;
                                            } else {
                                                updateArray[lineItemIndex] = 1;
                                            }
                                        } else {
                                            updateArray[lineItemIndex] = quantity;
                                        }
                                    } else {
                                        // Updates other bundle items 
                                        updateArray[lineItemIndex] = itemCurrentQty;
                                    }
                                });
                            }

                            var updatesObject = { updates: updateArray };


                            // Sends the updates as line item positions and quantities
                            fetch(window.routes.cartUrl + '/update.js', {
                            ...
                            const formData = {'updates': {[target.getAttribute('data-line-id')]: quantity}}
                            // this._updateSaleItem(formData, target, 41804283740314, 150, quantity, 1)

                            // this._updateSaleItem(formData, target, 41866862362778, 0, quantity,  10)
                            this._updateSaleItem(formData, target, 41866862362778, 100, quantity,  1)
                    ...
                    value: function _rerenderCart(elementToAnimate) {
                        ...
                        console.log('content: rerenderCart')
                            ...
                            if (_this35.options['drawer'] && elementToAnimate) {
                                ...
                                if (_this35.itemCount === 0) {
                                    $('.text_progress').html("You are $99 away from Free Domestic Shipping");
                    ...
                    value: function _replaceContent(html) {
                        ...
                        try {
                            var f = document.getElementById('total_price_of_cart').innerHTML;
                            document.getElementById('data-lion-get-cart-total').innerHTML = '<span class="material-icons" style="font-size:15px;vertical-align:top;color: #fcc49d">star_rate</span> Earn <span data-lion-points-for="' + f + '"></span> Perks Points for this order!';

                        } catch (error) { }
                        const textProgress = document.querySelector('.text_progress')
                        var nb_items_free_shipping = $('[data-collection-free-shipping="true"]').length;
                        var nb_items_free_non_shipping = $('[data-collection-free-shipping="false"]').length;
                        var ttl = $('#total_of_cart').html()?.substring(1).replace(",", "");
                        ttl = Number(ttl);
         
                        var pr = ttl * 100 / 99;
                        var rest = 99 - ttl;
                        // total_of_cart element is gone when no products are in cart
                        if(!ttl) {
                            textProgress.textContent = 'You are $99 away from Free Domestic Shipping'
                            return $('.progress_percent').css({ width: "0%" });
                        }
                        if (nb_items_free_shipping > 0 && nb_items_free_non_shipping === 0)
                            rest = 0;
                        if (Number(rest.toFixed(2)) <= 0) {
                            textProgress.textContent = "You have qualified for Free Domestic Shipping"
                            $('.progress_percent').css({ width: "100%" });
                        }
                        else {
                            textProgress.textContent = "You are $" + rest.toFixed(2) + " away from Free Domestic Shipping"
                            $('.progress_percent').css({ width: pr + "%" });
                        }
                    }
            ...
            var CollectionSection = function () {
                ...
                _createClass(CollectionSection, [{
                    ...
                    value: function _toggleTag(event) {
                        ...
                        var carousel_item = jQuery(element);
                        var is_not_carousel_item = true;
                        console.log(carousel_item.attr("data-element"))
                        if (carousel_item.attr("data-element") == "carousel-item") {
                            is_not_carousel_item = false;
                            this.temporaryTags = [];
                            this.temporaryTags.push(carousel_item.attr('data-tag'));
                            this._updateActiveTags();
                            this._commit();
                        }
                        if (is_not_carousel_item) {
                    ...
                    value: function _reloadProducts() {
                        ...
                        fetch(location.pathname + '?view=ajax&sort_by=' + this.currentSortBy, {
                            ...
                            response.text().then(function (content) {
                                ...
                                /* custom code JS for wishl icons after filter applied */
                                $(".wishl-quickview").each(function () {
                                    $(this).html($(".wishl_icon_" + $(this).attr("data-id")).html());
                                    $(this).click(function () {
                                        $(".wishl_icon_" + $(this).attr("data-id")).find("button").click();
                                    });
                                });
                                $(".wishl-grid").each(function () {
                                    $(this).html($(".wishl_icon_" + $(this).attr("data-id")).html());
                                    $(this).click(function () {
                                        $(".wishl_icon_" + $(this).attr("data-id")).find("button").click();
                                    });
                    ...
                    value: function _attachListeners() {
                        ...
                        $('.owl-carousel').on('click', '.owl-item> .item_collection ', this._toggleTagListener)
            ...
            var HeaderSection = function () {
                ...
                _createClass(HeaderSection, [{
                    ...
                    value: function _attachListeners() {
                        ...
                        this.delegateElement.on('mouseenter', '.Header__SecondaryNav .HorizontalList__Item, [aria-haspopup]', this._openMenu.bind(this), true);
                        ...
                        if (this.isTouch) {
                            ...
                            this.delegateElement.on('click', '.Header__SecondaryNav [aria-haspopup]', this._handleTouchMenu.bind(this));
                        }
                    ...
                    value: function _closeNavigation() {
                        ...
                        fastdom.mutate(function () {
                            ...
                            __WEBPACK_IMPORTED_MODULE_0__helper_Dom__["default"].nodeListToArray(_this50.element.querySelectorAll('.Header__SecondaryNav [aria-hidden="false"]')).forEach(function (item) {
                                item.setAttribute('aria-hidden', 'true');
                            });
                        });
                    ...
                    value: function _openMenu(event, target) {
                        ...
                        fastdom.mutate(function () {
                            target.classList.add('is-expanded');
                            __WEBPACK_IMPORTED_MODULE_0__helper_Dom__["default"].nodeListToArray(target.children, '.Header__SecondaryNav [aria-hidden="true"]').forEach(function (item) {
                                item.setAttribute('aria-hidden', 'false');
                            });
                            ...
                            __WEBPACK_IMPORTED_MODULE_0__helper_Dom__["default"].getSiblings(target, '.is-expanded').forEach(function (item) {
                                ...
                                __WEBPACK_IMPORTED_MODULE_0__helper_Dom__["default"].nodeListToArray(item.children, '.Header__SecondaryNav [aria-hidden="false"]').forEach(function (item) {
                                    item.setAttribute('aria-hidden', 'true');
                                });
                            });
                        });
                    ...
                    value: function _closeMenu(event, target) {
                        ...
                        fastdom.mutate(function () {
                            ...
                            __WEBPACK_IMPORTED_MODULE_0__helper_Dom__["default"].nodeListToArray(target.children, '.Header__SecondaryNav [aria-hidden="false"]').forEach(function (item) {
                                item.setAttribute('aria-hidden', 'true');
                            });
                        });
                    }
                    ...
                    value: function _verifyNavigationOverlap() {
                        ...
                        fastdom.measure(function () {
                            // To detect if the navigation is overlapping, we take the height of a single item and check if its height is taller than the parent
                            var mainTopMenu = _this51.element.querySelector('.Header__SecondaryNav');
...
/******* END CUSTOM CODE JS FOR TOGGLE MEGAMENU */



/******************************************* */
/******************************************* */
/******  CUSTOM CODE JS FOR TOGGLE FILTER MENU  */

/*** OnClick in button + to show the menu  */
function ControleToggleMenuFilterFunction(el) {
    elem = el.parent().parent().find('.Collapsible__Content').find('ul');
